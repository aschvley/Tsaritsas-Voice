...
module.exports = {
    ...
    async run(client, int, tools) {
        ...
        if (member.id != int.user.id && !isMod) {
            return tools.warn("You don't have permission to sync someone else's roles!");
        ...
        tools.syncLevelRoles(member, roleCheck).then(() => {
            ...
            if (roleCheck.missing.length) {
                replyStr.push(`Added: ${roleCheck.missing.map(x => `<@&${x.id}>`).join(" ")}`);
            }
            if (roleCheck.incorrect.length) {
                replyStr.push(`Removed: ${roleCheck.incorrect.map(x => `<@&${x.id}>`).join(" ")}`);
            }
            return int.reply(replyStr.join("\n"));
        }).catch((e) => {
            return int.reply(`Error syncing roles! ${e.message}`);
        });
    }
}
