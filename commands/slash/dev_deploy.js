    ...
    switch (arg.type) {
        ...
        case "number":
            return data.addIntegerOption(option => {
                ...
                if (!isNaN(arg.min)) {
                    option.setMinValue(arg.min);
                }
                if (!isNaN(arg.max)) {
                    option.setMaxValue(arg.max);
                }
                return option;
            });
        case "float":
        ...
        case "channel":
            return data.addChannelOption(option => {
                prepareOption(option, arg);
                if (arg.types) {
                    option.addChannelTypes(arg.types);
                } else if (arg.acceptAll) {
                    option.addChannelTypes([0, 2, 4, 5, 10, 11, 12, 13, 15, 16]); // lol
                } else {
                    option.addChannelTypes([Discord.ChannelType.GuildText, Discord.ChannelType.GuildAnnouncement]);
                }
                return option;
            });
        case "bool":
...
module.exports = {
    ...
    async run(client, int, tools) {
        ...
        // Defer la respuesta INMEDIATAMENTE si hay una interacción
        if (int) {
        ...
        if (!undeploy) {
            client.commands.forEach(cmd => {
                ...
                if (isPublic && metadata.dev) {
                    return;
                } else if (!isPublic && !metadata.dev) {
                    return;
                }
        ...
        if (isPublic && int) {
            ...
            rest.put(route, { body: interactionList })
                .then(() => {
                    int.editReply(`**${!undeploy ? `${interactionList.length} global commands registered!` : "Global commands cleared!"}** (Wait a bit, or refresh with Ctrl+R to see changes)`);
                    ...
                    int.editReply(`Error deploying global commands: ${e.message}`);
                });
        } else if (int) {
            ...
            const promises = serverIDs.map(id => {
                ...
                return rest.put(route, { body: interactionList })
                    .then(() => {
                        let msg = `Dev commands registered to ${id}!`;
                        return int.followUp({ content: undeploy ? "Dev commands cleared!" : id == int.guild.id ? "Dev commands registered!" : msg, flags: [Discord.MessageFlags.Ephemeral] });
                    }).catch(e => {
                        console.error(`Error deploying dev commands to ${id}: ${e.message}`);
                        return int.followUp({ content: `Error deploying dev commands to ${id}: ${e.message}`, flags: [Discord.MessageFlags.Ephemeral] });
                    });
            });

            Promise.all(promises)
                .then(() => {
                    int.editReply(`Successfully deployed/cleared dev commands to ${serverIDs.length} server(s).`);
                });
        } else {
