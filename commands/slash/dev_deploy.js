    ...
    switch (arg.type) {
        ...
        case "channel": {
            return data.addChannelOption(option => {
                ...
                if (arg.types) {
                    ...
                    option.addChannelTypes([0, 2, 4, 5, 10, 11, 12, 13, 15, 16]);
                } else {
        ...
        case "bool": {
            return data.addBooleanOption(option => {
                return prepareOption(option, arg);
            });
        }
        case "file": {
    ...
    async run(client, int, tools) {
            ...
            client.commands.forEach(cmd => {
                ...
                switch (metadata.type) {
                    ...
                    case "slash": {
                        ...
                        interactionList.push(data.toJSON ? data.toJSON() : data);
                        break;
                    }
                }
            });
        ...
        if (isPublic) {
            ...
            rest.put(route, { body: interactionList })
                .then(() => {
                    if (int) {
                        int.reply(`**${!undeploy ? `${interactionList.length} global commands registered!` : "Global commands cleared!"}** (Wait a bit, or refresh with Ctrl+R to see changes)`);
                    } else {
                        console.info("Global commands registered!");
                    }
            ...
            serverIDs.forEach(id => {
                ...
                rest.put(route, { body: interactionList })
                    .then(() => {
                        ...
                        if (int) {
                            if (undeploy) {
                                int.reply("Dev commands cleared!");
                            } else if (id == int.guild.id) {
                                int.reply("Dev commands registered!");
                            } else {
                                int.reply(msg);
                            }
                        } else {
