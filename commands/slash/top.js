...
module.exports = {
    ...
    async run(client, int, tools) {
        ...
        if (!db || !db.users || !Object.keys(db.users).length) {
            ...
            return tools.warn("The leaderboard is disabled in this server!" + (tools.canManageServer(int.member) ? `\nAs a moderator, you can still privately view the leaderboard here: ${lbLink}` : ""));
        }
        ...
        if (userSearch) {
            ...
            if (isNaN(foundRanking) || foundRanking < 0) {
                return tools.warn(int.user.id == userSearch.user.id ? "You aren't on the leaderboard!" : "This member isn't on the leaderboard!");
            } else {
                pageNumber = Math.floor(foundRanking / pageSize) + 1;
